    ...
    <div className="space-y-6 pb-16">
      <div className="rounded-[32px] border border-[color:var(--line)] bg-[color:var(--surface)] px-6 py-7 shadow-[0_25px_60px_rgba(15,23,42,0.08)]">
        ...
        <p className="mt-3 text-sm leading-relaxed" style={{ color: 'var(--muted)' }}>
          Hver badge har en hemmelig udfordring. Saml dem alle – eller prøv i det mindste at finde ud af,
          hvorfor vi synes de er sjove.
        </p>
      ...
      <div className="space-y-4">
        {ACHIEVEMENTS.map((achievement) => {
          const unlocked = !!userData?.achievements?.[achievement.id]
          return (
            <AchievementLabel key={achievement.id} achievement={achievement} unlocked={unlocked} />
          )
        })}
      </div>
    </div>
