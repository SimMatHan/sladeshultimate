...
export function SladeshProvider({ children }) {
    ...
    // Automatically mark challenges as seen when they become completed or failed
    // This prevents the scanner from re-appearing after the user has finished
    useEffect(() => {
        challenges.forEach(challenge => {
            if ((challenge.status === SLADESH_STATUS.COMPLETED || challenge.status === SLADESH_STATUS.FAILED) &&
                challenge.receiverId === currentUser?.uid) {
                scannerSeenChallengesRef.current.add(challenge.id);
            }
        });
    }, [challenges, currentUser]);
